...
CEasyCashDoc* CEasyCashDoc::Jahreswechsel(int land = 0)
{
	...
	csSlashAltesJahr.Format("/%04d", nJahr);
	while (pB)
	{
		if (pB->AbschreibungJahre > 1 && pB->AbschreibungRestwert > pB->GetBuchungsjahrNetto(this))
		{
			...
			**ppB = *pB;
			...
			(*ppB)->AbschreibungRestwert -= (*ppB)->GetBuchungsjahrNetto(this);
			(*ppB)->AbschreibungNr++;
		...
		if (file_version < 7)
		{
...

			{
				if (nJahr < 2004)
				{
					...
					AfxMessageBox("Antike .eca-Buchungsdatei: Halbjahres-AfA wurde angenommen.");
				}
				else
				{
					AbschreibungGenauigkeit = MONATSGENAUE_AFA;
					AfxMessageBox("Antike .eca-Buchungsdatei: Monatsgenaue AfA wurde angenommen.");
				}
			}
		}
		else
...
CString CEasyCashDoc::GetFormularwertByIndex(XDoc *pFormular, int nIndex, LPCSTR sFilter)
{
	...
	if (child && !child->value.IsEmpty())
	{
		...
		if (!pDarstellenBeiNullwert || stricmp(pDarstellenBeiNullwert, "ja"))
			if (csFeldinhalt == (CString)"0,00" || csFeldinhalt == "0")
				csFeldinhalt = "";
