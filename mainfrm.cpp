...
CMainFrame::CMainFrame()
{
	...
	m_pPlugins = NULL;
	m_pInfoButton = NULL;
	m_pEinnahmeButton = NULL;
	m_pAusgabeButton = NULL;
...
void CMainFrame::Add_Category1()
{
	...
	m_pEinnahmeButton = new CMFCRibbonButton(ID_EDIT_EINNAHME_BUCHEN, "Einnahme\ne", 5, 5);
#ifdef USE_ECTENGINE
	// Split-Button: Klick auf den Knopf bucht wie gehabt, der Pfeil oeffnet ein
	// Dropdown mit den Buchungsvorlagen (befuellt in UpdateBuchungsvorlagenMenu).
	// Das Menue-Resource ist nur ein Platzhalter -- der Inhalt wird dynamisch
	// ersetzt (analog zum Konten-Filter-Knopf).
	m_pEinnahmeButton->SetMenu(IDR_FILTER_KONTO, TRUE);
	m_pEinnahmeButton->RemoveAllSubItems();
#endif
	m_pEinnahmeButton->SetAlwaysLargeImage();
	pPanel1->Add(m_pEinnahmeButton);

	m_pAusgabeButton = new CMFCRibbonButton(ID_EDIT_AUSGABE_BUCHEN, "Ausgabe\na", 6, 6);
#ifdef USE_ECTENGINE
	m_pAusgabeButton->SetMenu(IDR_FILTER_KONTO, TRUE);
	m_pAusgabeButton->RemoveAllSubItems();
#endif
	m_pAusgabeButton->SetAlwaysLargeImage();
	pPanel1->Add(m_pAusgabeButton);
...
#ifdef USE_ECTENGINE
void CMainFrame::UpdateBuchungsvorlagenMenu()
{
	// Dropdown-Menues der Einnahme-/Ausgabe-Knoepfe mit den aktuellen
	// Buchungsvorlagen (Buchungsposten) fuellen -- Nummer vorangestellt,
	// getrennt nach Einnahmen/Ausgaben. Die Auswahl eines Eintrags oeffnet
	// den Buchen-Dialog mit vorgewaehlter Vorlage (CEasyCashView::OnCmdMsg).
	if (m_pEinnahmeButton) m_pEinnahmeButton->RemoveAllSubItems();
	if (m_pAusgabeButton)  m_pAusgabeButton->RemoveAllSubItems();

	int nEinnahmen = 0, nAusgaben = 0;
	for (int i = 0; i < 100; i++)
	{
		ECT_Preset preset;
		if (!ECT_HolePreset(i, &preset)) continue;
		if (!preset.text[0]) continue;		// leerer Slot

		CString csText;
		csText.Format("%02d  %s", i, preset.text);
		...
		if (preset.ausgabe)
		{
			if (m_pAusgabeButton)
			{
				m_pAusgabeButton->AddSubItem(new CMFCRibbonButton(
					ID_CMD_VORLAGE_AUSGABE_BASE + i, (LPCTSTR)csText));
				nAusgaben++;
			}
		}
		else
		{
			if (m_pEinnahmeButton)
			{
				m_pEinnahmeButton->AddSubItem(new CMFCRibbonButton(
					ID_CMD_VORLAGE_EINNAHME_BASE + i, (LPCTSTR)csText));
				nEinnahmen++;
			}
		}
	}
...
void CMainFrame::OnFileMandanten() 
{
...

	{
		...
		// Sonderfall: der letzte Mandant wurde geloescht -> zurueck in den
		// Nicht-Mandanten-Modus. Die Mandanten-Sektion ist oben bereits geleert;
		// jetzt noch dessen Datenverzeichnis als regulaeres Datenverzeichnis setzen
		// und die passende easyct.ini aktiv machen (wie beim Mandantenwechsel).
		if (szNichtMandantVerz[0] != '\0')
		{
			...
			strcpy(szIniFileName, szNichtMandantVerz);
			strcat(szIniFileName, "\\easyct.ini");
			SetIniFileName(szIniFileName);	// laedt auch den Einstellungs-Cache neu
			char last_file[500];
